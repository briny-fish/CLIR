...
outfile = open('/home/zongwz/%s%sdocsBertlogen-zh-%s.txt'% (paramdict['model'],param,time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())), 'w+', encoding='utf-8')
...
exfilepkl = '/home/zongwz/ex_docs_inputs.pkl'
...
if os.path.exists(exfilepkl):
    ...
    ex_docs_inputs = dataProc.compute_input_arrays_single(ex_docs, input_categories, tokenizer, MAX_SEQUENCE_LENGTH)
...
def get_ex_docs(queryidl,queryidr,batch_size):
    ...
    for x in range(paramdict['exout_num']):
        ...
        tmp[x][0] = [ex_docs_inputs[0][0] for i in range(batch_size)]
        tmp[x][1] = [ex_docs_inputs[1][0] for i in range(batch_size)]
        tmp[x][2] = [ex_docs_inputs[2][0] for i in range(batch_size)]
    return tmp
